...
#' @export
GenerateEmail <- function(max.date, bay.area.rt) {
  ...
  x <- as.data.table(readxl::read_excel("Forecasts/SFresidents.xlsx"))
  x[, date := as.Date(date)]
  ...
  bay.area.rt[county == "SFresidents", Re := curr.rt]

  cat("Estimated Re (14 days ago) ", GetStr(prev.rt, curr.rt), " with data through ", ShortDate(max.date), ".", sep = "")


  today.plus30 <- max.date + 30
  # cat("\n30 day forecast (", ShortDate(today.plus30), ") Hospitalization (SF residents): ", HospStr(x[date == today.plus30, hosp]), "\n", sep = "")
  cat("\n")
  print(bay.area.rt)
  cat("\n")
