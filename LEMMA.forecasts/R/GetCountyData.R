...
GetCountyData <- function(include.regions = FALSE, remove.holidays = TRUE, states = FALSE, remote = FALSE) {
  ...
  dt <- dt[, .(county, date = as.Date(todays_date), hosp.conf = hospitalized_covid_confirmed_patients, hosp.pui = 0, icu.conf = 0, icu.pui = 0)] #icu and pui no longer used
  dt[date %in% as.Date(c("2023-05-11", "2023-05-12")), hosp.conf := NA] #data problems
  dt[date %in% as.Date(c("2023-05-11", "2023-05-12")), hosp.pui := NA] #data problems
  ...
  if (T) {
    ...
    #assume last 4 days of cases, 30 days of deaths are not reliable
    max.date <- deaths.cases[, max(date)]
    #case.dt <- deaths.cases[date >= as.Date("2020/9/25") & date < (max.date - 4), .(date, county, cases.conf = cases, cases.pui = NA_real_)]
    case.dt <- deaths.cases[date >= as.Date("2020/9/25") & date < (max.date - 5), .(date, county, cases.conf = as.double(cases), cases.pui = NA_real_)] #1/11/22 - switch to 5 day; 1/17/22 switch to 6 day; 3/4/22 back to 5 day

    if (F) {
      holiday.set <- as.Date(c("2020/11/26", "2020/11/27", "2020/12/25", "2021/1/1", "2021/9/6", "2021/11/25", "2021/11/26"))
      # holiday.set <- as.Date(c("2020/11/26", "2020/11/27", "2020/12/25", "2021/1/1", "2021/9/6", "2021/11/28", "2021/11/22", "2021/11/23", "2021/11/24", "2021/11/25", "2021/11/26", "2021/11/27"))
      if (remove.holidays) {
        case.dt[date %in% holiday.set, cases.conf := NA_real_] #remove major holidays before and after data.table::frollmean
      }
      ...
      # case.dt[weekdays(date) %in% c("Saturday", "Sunday"), cases.conf := NA_real_]
      holiday.set <- GetHolidays()
      case.dt[date %in% holiday.set, cases.conf := NA_real_]
    }
  ...
  #source for admits doesn't match CHDR, exclude for now
  ...
  always.include <- "" #c("Marin", "Santa Cruz") #run these even if hosp < 1
  county.dt <- county.dt[date >= as.Date("2020/3/1")]
  max_hosp_date <- county.dt[!is.na(hosp.conf), max(date)]
  county.dt[, mean10 := mean(hosp.conf[date >= (max_hosp_date - 10)], na.rm=T), by = "county"]
  ...
  #Nevada has weird hosp data for a few days late March, early April
  county.dt[county == "Nevada" & date < as.Date("2020/4/3"), hosp.conf := NA]
  county.dt[county == "Nevada" & date < as.Date("2020/4/3"), hosp.pui := NA]

  #Misc data problems
  county.dt[county == "Inyo" & date=="2020-03-31", hosp.pui := NA_real_]
  county.dt[county == "Imperial" & date=="2022-12-26" & hosp.conf > 60, c("hosp.conf", "hosp.pui") := list(NA, NA)]
...
GetHolidays <- function() {
  #post Omicron holidays
  as.Date(c("2021/11/25", "2021/11/26", "2021/12/24", "2021/12/25", "2021/12/31", "2022/1/1", "2022/1/17", "2022/2/21",  "2022/5/30", "2022/6/20", "2022/7/4", "2022/9/5", "2022/11/11", "2022/11/25", "2022/11/26", "2022/12/24", "2022/12/25",  "2022/12/26", "2023/1/1", "2023/1/2"))
}
...
GetStateData <- function(remove.holidays = TRUE) {
  ...
  if (T) {
    ...
    for (i in ar[date > hosp.dt.prev[, max(date) - 10], `Archive Link`]) {
  ...
  #cases (NYT)
  x <- data.table::fread("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv")
  x[, date := as.Date(date)]
  setnames(x, "state", "StateName")
  x <- merge(x, state.abbr.dt)
  x <- x[date >= as.Date("2020/9/25")]
  x[, cases.conf := c(NA, diff(as.numeric(cases))), by = state]
  x[, cases.pui := NA_real_]
  setkey(x, state, date)
