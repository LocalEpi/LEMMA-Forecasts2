...
GetDosesData <- function(states = FALSE, remote = FALSE) {
  if (states) {
    ...
    doses.dt[, dose1 := pmax(dose1, 0)]
    doses.dt[, dose2 := pmax(dose2, 0)]
    doses.dt[, doseJ := pmax(doseJ, 0)]
  } else {
...
#' @export
SFDosesToAWS <- function(cair_date = Sys.Date(), chdr_date = Sys.Date() - 1) {
  filestr <- paste0("~/Documents/MissionCovid/CAIR_Summary_Report_", format(cair_date, "%m%d%Y"), ".csv")
  d <- fread(filestr)
...
ReadSFDoses <- function(print_outdate = FALSE) {
  ...
  if (print_outdate) {
    ...
    dt <- data.table(name = c("all", "12+", "12-64", "65+"),
                     ...
                     denom = c(833401, 759819, 626987, 132832))
    dt[, frac_vax := numer / denom]
    print(dt, digits = 3)

    cat("fully vaccinated:\n")
    under65 <- d[dose_num %in% c("2", "J"), sum(count_under65)]
    over65 <- d[dose_num %in% c("2", "J"), sum(count_65plus)]
    total <- under65 + over65
