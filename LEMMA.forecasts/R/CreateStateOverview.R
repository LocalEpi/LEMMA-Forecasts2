...
CreateOverview <- function(lemma.set, writedir = NULL) {
  ...
  if (is.null(writedir)) {
    pdf("DELTAONLY_Forecasts/DELTAONLY_StateOverview.pdf", width = 11, height = 8.5)
  } else {
...
RtMap <- function(max.date, county.rt, writedir = NULL) {
  ...
  if (is.null(writedir)) {
    bay.area <- c("SFresidents", "San Mateo",  "Alameda", "Contra Costa", "Santa Clara", "Marin")
    bay.area.rt <- county.rt[county %in% bay.area, .(county, Re = round(Rt, 2))]
  ...
  invisible(bay.area.rt)
}


AggregateState <- function(region, county.set, county.pop, max.date, path, postfix) {
  ...
  for (county1 in county.set) {
    ...
    proj <- ReadForecast(paste0(path, "/", county1, postfix, ".xlsx"))
    proj[, rt := rt * cur_frac]
    proj[, seroprev := seroprev * cur_frac]
    proj[, relativeEffectiveContactRate := relativeEffectiveContactRate * cur_frac]
    if (is.null(total_proj)) {
      total_proj <- proj
    } else {
      stopifnot(all.equal(proj$date, total_proj$date))
      for (i in setdiff(names(total_proj), "date")) {
        if (i %in% c("rt", "seroprev", "relativeEffectiveContactRate")) {
  ...
  openxlsx::write.xlsx(list(projection = total_proj), file = paste0(path, "/", region, postfix, ".xlsx"), overwrite = T)

  title1 <- paste(path, region, postfix)
  dt <- merge(total_proj, county.dt[county %in% names(county.pop), .(hosp.conf = sum(hosp.conf), icu.conf = sum(icu.conf), cases.conf = sum(cases.conf)), by = "date"], all.x = T, by = "date")
  print(ggplot(dt, aes(x = date)) + geom_line(aes(y = hosp)) + geom_point(aes(y = hosp.conf), na.rm = T) + ggtitle(title1))
  print(ggplot(dt[abs(date - max.date) < 60], aes(x = date)) + geom_line(aes(y = hosp)) + geom_point(aes(y = hosp.conf), na.rm = T) + ggtitle(title1))

  print(ggplot(dt, aes(x = date)) + geom_line(aes(y = cases)) + geom_point(aes(y = cases.conf), na.rm = T) + ggtitle(title1))
  print(ggplot(dt[abs(date - max.date) < 60], aes(x = date)) + geom_line(aes(y = cases)) + geom_point(aes(y = cases.conf), na.rm = T) + ggtitle(title1))

  print(ggplot(dt, aes(x = date)) + geom_line(aes(y = icu)) + geom_point(aes(y = icu.conf), na.rm = T) + ggtitle(title1))
  print(ggplot(dt[abs(date - max.date) < 60], aes(x = date)) + geom_line(aes(y = icu)) + geom_point(aes(y = icu.conf), na.rm = T) + ggtitle(title1))

}

ReadForecast <- function(filestr, sheet = "projection") {
  proj <- as.data.table(read_excel(filestr, sheet))
  if (sheet == "projection" ) {
...
CreateOverviewScen <- function(county.dt, county.set, max.date, county.pop, max.plot.date = Sys.Date() + 60) {
  FormatFilename <- function(f) {
    f <- sub("./", "", f, fixed = T)
    f <- sub(".xlsx", "", f, fixed = T)
    return(f)
  }

  county.rt <- data.table(county = county.set, Rt = NA_real_)
  pdf("Forecasts/StateOverview_scen.pdf", width = 11, height = 8.5)
  for (county1 in county.set) {

    date_range <- seq.Date(as.Date("2021/6/1"), max.plot.date, by = "day")
    county.dt2 <- county.dt[date %in% date_range & county == county1]

    gg <- list()

    dt_plot <- list(NULL, NULL, NULL)
    for (boosters1 in boosters_set) {
      for (contacts1 in contacts_set) {
        f <- paste0("Scenarios/", county1, "_", boosters1, "_", contacts1, ".xlsx")
        if (!file.exists(f)) {
          cat("Not found: ", f, "\n")
          next
        }
        x <- ReadForecast(f)
        filestr <- FormatFilename(f)
        if (is.na(county.rt[county == county1, Rt])) {
          county.rt[county == county1, Rt := x[date == max.date - 14, rt]]
        }
        for (i in 1:3) {
          if (i == 1) {
            z <- x[, .(date, value = hosp)]
          } else if (i == 2) {
            z <- x[, .(date, value = cases)]
          } else if (i == 3) {
            z <- x[, .(date, value = icu)]
          }
          z[, boosters := boosters1]
          z[, contacts := contacts1]
          dt_plot[[i]] <- rbind(dt_plot[[i]], z)
        }
      }
    }

    pop_scale <- county.pop[county1] / 100000
    for (i in 1:3) {
      if (i == 1) {
        actual <- county.dt2[, .(date, y = hosp.conf + 0.3 * hosp.pui, lab = "")]
        ylab1 <- "Hospital Census per 100k"
      } else if (i == 2) {
        actual <- county.dt2[, .(date, y = cases.conf)]
        ylab1 <- "Cases per 100k"
      } else if (i == 3) {
        actual <- county.dt2[, .(date, y = icu.conf + 0.3 * icu.pui)]
        ylab1 <- "ICU Census per 100k"
      }

      dt_plot[[i]][, contacts := factor(contacts, levels = contacts_set)]
      dt_plot[[i]][, boosters := factor(boosters, levels = boosters_set)]

      gg[[i]] <- ggplot(dt_plot[[i]][date %in% date_range], aes(x=date)) + geom_line(aes(y = value / pop_scale, color = boosters, lty = contacts), na.rm = T) + xlab("") + ylab(ylab1) + annotate("point", x = actual[, date], y = actual[, y] / pop_scale, na.rm = T) + ggtitle(county1) + scale_y_log10() #coord_cartesian(ylim = c(0, actual[, max(y / pop_scale, na.rm = T) * 1.5]))
    }

    print(ggpubr::ggarrange(gg[[1]], gg[[2]], gg[[3]], nrow = 3))
  }
  dev.off()
  return(county.rt)
}
